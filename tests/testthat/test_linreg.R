...
test_that("print() method works", {
  ...
  expect_output(linreg_mod$print(),writeLines("linreg\\(formula = Petal\\.Length ~ Sepal\\.Width \\+ Sepal\\.Length, data = iris\\)"))
...
test_that("summary() method works", {
  ...
  expect_output(linreg_mod$summary(), writeLines("\\(Intercept\\)( )*-2.5[0-9]*( )*0.5[0-9]*( )*-4.4[0-9]*( )*.*( )*\\*\\*\\*"))  
  expect_output(linreg_mod$summary(), writeLines("Sepal.Width( )*-1.3[0-9]*( )*0.1[0-9]*( )*-10.9[0-9]*( )*.*( )*\\*\\*\\*"))
  expect_output(linreg_mod$summary(), writeLines("Sepal.Length( )*1.7[0-9]*( )*0.0[0-9]*( )*27.5[0-9]*( )*.*( )*\\*\\*\\*"))
  expect_output(linreg_mod$summary(), writeLines("Residual standard error: 0.6[0-9]* on 147 degrees of freedom"))
})
